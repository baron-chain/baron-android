...
//                result.addresses.add(resource);
    ...
    public static BigDecimal getRealBlockPerYear(BaseChain chain) {
        if (chain != null) {
            if (ChainFactory.getChain(chain).getRealBlockTime() == BigDecimal.ZERO) {
                return BigDecimal.ZERO;
            }
        }
        return YEAR_SEC.divide(ChainFactory.getChain(chain).getRealBlockTime(), 2, RoundingMode.DOWN);
    }

    public static String getExplorer(BaseChain basechain) {
        return ChainFactory.getChain(basechain).getExplorer();
    }

    public static String getTxExplorer(BaseChain basechain, String hash) {
        if (hash != null) {
            return getExplorer(basechain) + "txs/" + hash;
        }
        return "";
    }
    ...
    public static BigDecimal getEstimateGasFeeAmount(Context c, BaseChain basechain, int txType,  int valCnt) {
        return ChainFactory.getChain(basechain).setEstimateGasFeeAmount(c, basechain, txType, valCnt);
    }
