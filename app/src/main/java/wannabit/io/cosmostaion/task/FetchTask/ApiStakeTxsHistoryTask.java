    ...
    protected TaskResult doInBackground(String... strings) {
        try {
            ...
            if (mChain.equals(BaseChain.COSMOS_MAIN)) {
                ...
                Response<ArrayList<ResApiNewTxListCustom>> response = ApiClient.getCrescentApi(mApp).getNewStakeTxsCustom(mAddress, mValOpAddress, "50").execute();
                if (response.isSuccessful() && response.body() != null) {
                    mResult.resultData = response.body();
                    mResult.isSuccess = true;
                } else {
                ...
                Response<ArrayList<ResApiNewTxListCustom>> response = ApiClient.getMantleApi(mApp).getNewStakeTxsCustom(mAddress, mValOpAddress, "50").execute();
                if (response.isSuccessful() && response.body() != null) {
                    mResult.resultData = response.body();
                    mResult.isSuccess = true;
                } else {
