...
import static wannabit.io.cosmostaion.base.BaseConstant.BLOCK_TIME_COMDEX;
import static wannabit.io.cosmostaion.base.BaseConstant.COINGECKO_COMDEX_MAIN;
import static wannabit.io.cosmostaion.base.BaseConstant.COMDEX_GAS_RATE_AVERAGE;
import static wannabit.io.cosmostaion.base.BaseConstant.COMDEX_GAS_RATE_LOW;
import static wannabit.io.cosmostaion.base.BaseConstant.COMDEX_GAS_RATE_TINY;
...
import static wannabit.io.cosmostaion.base.BaseConstant.EXPLORER_COMDEX_MAIN;
...
public class Comdex extends Chain {

    @Override
    public BaseChain getChain() { return COMDEX_MAIN; }

    @Override
    public ArrayList<BaseChain> getChains() { return Lists.newArrayList(COMDEX_MAIN); }
    ...
    @Override
    public BigDecimal getRealBlockTime() { return BLOCK_TIME_COMDEX; }

    @Override
    public String getExplorer() { return EXPLORER_COMDEX_MAIN; }
    ...
    public String getDpAddress(byte[] converted) {
        return bech32Encode("comdex".getBytes(), converted);
    }
    ...
    public void setInfoImg(ImageView imageView, int type) {
        if (type == 0) {
            imageView.setImageResource(R.drawable.chain_comdex);
        } else if (type == 1) {
    ...
    public boolean isValidChainAddress(String address, BaseChain baseChain) {
        if (address.startsWith("comdex1") && baseChain.equals(getChain())) { return true; }
        else { return false; }
    }

    @Override
    public String getDefaultRelayerImg() { return COMDEX_UNKNOWN_RELAYER; }
    ...
    public void setLayoutColor(Context c, int length, LinearLayout[] wordsLayer) {
        wordsLayer[length].setBackground(c.getDrawable(R.drawable.box_round_comdex));
    }
    ...
    public void setMainIntent(MainActivity mainActivity, int sequence) {
        if (sequence == 1) {
            mainActivity.startActivity(new Intent(Intent.ACTION_VIEW , Uri.parse(COINGECKO_COMDEX_MAIN)));
        } else if (sequence == 2) {
            mainActivity.startActivity(new Intent(Intent.ACTION_VIEW , Uri.parse("https://comdex.one/home")));
        } else if (sequence == 3) {
            mainActivity.startActivity(new Intent(Intent.ACTION_VIEW , Uri.parse("https://blog.comdex.one")));
        }
    }
