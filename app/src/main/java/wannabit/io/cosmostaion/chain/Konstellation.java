...
import static wannabit.io.cosmostaion.base.BaseConstant.BLOCK_TIME_KONSTELLATION;
import static wannabit.io.cosmostaion.base.BaseConstant.COINGECKO_KI_MAIN;
import static wannabit.io.cosmostaion.base.BaseConstant.COINGECKO_KONSTELL_MAIN;
import static wannabit.io.cosmostaion.base.BaseConstant.EXPLORER_KI_MAIN;
import static wannabit.io.cosmostaion.base.BaseConstant.EXPLORER_KONSTELL_MAIN;
...
import static wannabit.io.cosmostaion.base.BaseConstant.KONSTELL_GAS_RATE_AVERAGE;
import static wannabit.io.cosmostaion.base.BaseConstant.KONSTELL_GAS_RATE_LOW;
import static wannabit.io.cosmostaion.base.BaseConstant.KONSTELL_GAS_RATE_TINY;
...
public class Konstellation extends Chain {

    @Override
    public BaseChain getChain() { return KONSTELL_MAIN; }

    @Override
    public ArrayList<BaseChain> getChains() { return Lists.newArrayList(KONSTELL_MAIN); }
    ...
    @Override
    public BigDecimal getRealBlockTime() { return BLOCK_TIME_KONSTELLATION; }

    @Override
    public String getExplorer() { return EXPLORER_KONSTELL_MAIN; }
    ...
    public String getDpAddress(byte[] converted) {
        return bech32Encode("darc".getBytes(), converted);
    }
    ...
    public void setDpMainDenom(Context c, TextView denomTxt) {
        denomTxt.setTextColor(c.getResources().getColor(R.color.colorKonstellation));
        denomTxt.setText(c.getString(R.string.s_konstellation));
    }
    ...
    public void setInfoImg(ImageView imageView, int type) {
        if (type == 0) {
            imageView.setImageResource(R.drawable.chain_konstellation);
        } else if (type == 1) {
    ...
    public boolean isValidChainAddress(String address, BaseChain baseChain) {
        if (address.startsWith("darc1") && baseChain.equals(getChain())) { return true; }
        else { return false; }
    }

    @Override
    public String getDefaultRelayerImg() { return KONSTELL_UNKNOWN_RELAYER; }
    ...
    public void setLayoutColor(Context c, int length, LinearLayout[] wordsLayer) {
        wordsLayer[length].setBackground(c.getDrawable(R.drawable.box_round_konstellattion));
    }
    ...
    public void setMainIntent(MainActivity mainActivity, int sequence) {
        if (sequence == 1) {
            mainActivity.startActivity(new Intent(Intent.ACTION_VIEW , Uri.parse(COINGECKO_KONSTELL_MAIN)));
        } else if (sequence == 2) {
            mainActivity.startActivity(new Intent(Intent.ACTION_VIEW , Uri.parse("https://konstellation.tech/")));
        } else if (sequence == 3 ) {
            mainActivity.startActivity(new Intent(Intent.ACTION_VIEW , Uri.parse("https://konstellation.medium.com/")));
        }
    }
